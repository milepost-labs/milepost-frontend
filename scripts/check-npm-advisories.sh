...
# Scope
# -----
# The repository's lockfile is audited with `--omit=dev`: the shipped
...
# app handles wallet signing, so a compromised production dependency
# reaches transaction construction — that is the failure this check
# exists for.
#
# Threshold
# ---------
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/milepost-frontend-npm-audit.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
